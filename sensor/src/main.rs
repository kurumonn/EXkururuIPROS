...
/// Module names must be alphanumeric + underscore only, max 64 chars.
...
fn kernel_hardening_enabled() -> bool {
    env::var("IPS_ALLOW_KERNEL_HARDENING")
        .ok()
        .map(|v| matches!(v.trim().to_lowercase().as_str(), "1" | "true" | "yes" | "on"))
        .unwrap_or(false)
}

/// Apply modprobe blacklist + rmmod for PinTheft (RDS zerocopy LPE) and similar kernel CVEs.
/// target_value: comma-separated module names in unload order (e.g. "rds_tcp,rds").
/// Requires IPS_ALLOW_KERNEL_HARDENING=1 and apply_mode="nft".
fn apply_kernel_module_blacklist(cfg: &Config, target_value: &str) -> ApplyResult {
    if !kernel_hardening_enabled() {
        return ApplyResult {
            status: "skipped".to_string(),
            meta: json!({
                "reason": "IPS_ALLOW_KERNEL_HARDENING not enabled",
                "hint": "set IPS_ALLOW_KERNEL_HARDENING=1 to enable auto kernel hardening",
            }),
        };
    }

    let modules: Vec<&str> = target_value
        .split(',')
        .map(|s| s.trim())
        .filter(|s| !s.is_empty())
        .collect();
    ...
    for &module in &modules {
        if !is_safe_module_name(module) {
            return ApplyResult {
                ...
                meta: json!({"error": format!("unsafe module name rejected: {module}")}),
            };
        }
    }
    ...
    // Write /etc/modprobe.d/ips-auto-kernel-hardening.conf
    let conf_lines: String = modules
        .iter()
        .map(|&m| format!("install {m} /bin/false\n"))
        .collect();
    let conf_content = format!(
        "# Auto-generated by EXkururuIPROS sensor — do not edit manually\n\
         # PinTheft kernel LPE mitigation (RDS zerocopy + io_uring)\n\
         {conf_lines}"
    );
    let conf_path = "/etc/modprobe.d/ips-auto-kernel-hardening.conf";
    if let Err(e) = std::fs::write(conf_path, &conf_content) {
        return ApplyResult {
            status: "failed".to_string(),
            meta: json!({"error": format!("write {conf_path} failed: {e}")}),
        };
    }

    // Attempt to unload modules (order matters: dependents first)
    let mut unload_results: Vec<serde_json::Value> = Vec::new();
    for &module in &modules {
        ...
        match output {
            ...
            Err(e) => {
                unload_results.push(json!({"module": module, "error": e.to_string()}));
            }
        }
    }

    // Best-effort initramfs update (suppresses errors — not fatal)
    let initramfs_updated = if Command::new("update-initramfs")
        .args(["-u", "-k", "all"])
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .status()
    ...
    ApplyResult {
        ...
        meta: json!({
            ...
            "initramfs_updated": initramfs_updated,
        }),
    }
}
...
fn apply_actions(cfg: &Config, actions: &[PendingAction]) -> Vec<(i64, ApplyResult)> {
    ...
    // kernel_module_blacklist is handled first and independently of apply_mode/nft gate.
    let mut results: Vec<(i64, ApplyResult)> = Vec::new();
    let mut ip_actions: Vec<&PendingAction> = Vec::new();
    for action in actions {
        if action.target_type == "kernel_module_blacklist" {
            let r = apply_kernel_module_blacklist(cfg, &action.target_value);
            results.push((action.id, r));
        } else {
            ip_actions.push(action);
        }
    }

    if ip_actions.is_empty() {
        return results;
    }
    ...
    let mut outputs: Vec<Option<ApplyResult>> = vec![None; ip_actions.len()];
    let mut valid_inputs: Vec<(usize, String, u64)> = Vec::new();

    for (idx, action) in ip_actions.iter().enumerate() {
...
async fn main() -> Result<()> {
    ...
    loop {
        let cycle = async {
            ...
            // Fetch and apply actions.
            // kernel_module_blacklist actions are always processed (WAF gate bypassed)
            // so that PinTheft auto-mitigation works even if WAF is in observe mode.
            let actions = fetch_pending_actions(&client, &cfg).await?;
            let (kernel_actions, ip_only_actions): (Vec<_>, Vec<_>) =
                actions.iter().partition(|a| a.target_type == "kernel_module_blacklist");
            let mut all_applied: Vec<(i64, ApplyResult)> = Vec::new();
            if !kernel_actions.is_empty() {
                all_applied.extend(apply_actions(&cfg, &kernel_actions));
            }
            if current_waf_enabled && !ip_only_actions.is_empty() {
                all_applied.extend(apply_actions(&cfg, &ip_only_actions));
            } else if !current_waf_enabled && !ip_only_actions.is_empty() {
                println!("waf is disabled by server policy; skipping ip block actions");
            }
            if !all_applied.is_empty() {
                let mut ack_payloads: Vec<(i64, AckPayload)> = Vec::with_capacity(all_applied.len());
                for (action_id, result) in all_applied {
                    ack_payloads.push((
                        action_id,
                        AckPayload {
